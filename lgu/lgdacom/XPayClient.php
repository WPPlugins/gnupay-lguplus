...
class XPayClient
{
	...
	public function __construct($home_dir, $mode="real")
	{
        ...
        // log_dir 재설정
        $this->config["log_dir"] = gp_lguplus_upload_path().'/log';    //로그파일 임의 수정
		...
		if (!file_exists($this->config["log_dir"])) {
			wp_mkdir_p($this->config["log_dir"]);
		}
...
//		$this->log("session id = ".session_id().$_SESSION['tx_counter'], LGD_LOG_FATAL);
	...
	{
		$now = date("YmdHis", GNUPAY_LGUPLUS_SERVER_TIME);
	...
	function Log($msg, $level=LGD_LOG_FATAL)
	{
		if ($level > $this->config["log_level"])
			return;
		$err_msg = date("Y-m-d H:i:s", GNUPAY_LGUPLUS_SERVER_TIME)." [".$this->err_label[$level]."] [".$this->TX_ID."] ".$msg."\n";
